	...
	private void computeEntropy() {
		...
		for (TestFitnessFunction testFitnessFunction : this.getTestCase().getCoveredGoals()) {
			if (testFitnessFunction instanceof BranchCoverageTestFitness) {
				String methodSignature = testFitnessFunction.getTargetClass() + "." + testFitnessFunction.getTargetMethod();

				coverages.put(methodSignature, coverages.getOrDefault(methodSignature, 0) + 1);
			}
		}

		// For each covered branchless method (in coveredMethods and not in coverages), we put the number of covered
		// branches at 1.
		for (String coveredMethod : coveredMethods) {
			if (!coverages.containsKey(coveredMethod))
				coverages.put(coveredMethod, 1);
		}

//    LoggingUtils.getEvoLogger().error(coverages.keySet().stream().reduce("", (s, s2) -> s + ", " + s2));
		...
		// set the value of the computed entropy also in the TC todo: improve this at some point
		this.test.setCoverageEntropy(-entropy);
	}
	...
	private void setCohesion(double cohesion) {
	...
	private void setCoupling(double coupling) {
